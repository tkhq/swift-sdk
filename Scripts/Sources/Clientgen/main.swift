...
// Converts a swagger definition name like "v1CreateSubOrganizationResultV8"
// to a formatted key name like "createSubOrganizationResultV8"
func formatDefinitionKey(_ key: String) -> String {
    let regex = try! NSRegularExpression(pattern: "^v\\d+")
    let range = NSRange(key.startIndex..<key.endIndex, in: key)
    let stripped = regex.stringByReplacingMatches(in: key, range: range, withTemplate: "")
...
// Scans swagger definitions for versioned Intent types (e.g. v1CreateOauthProvidersIntentV2)
// and builds a fallback map: ACTIVITY_TYPE_FOO -> ACTIVITY_TYPE_FOO_V2.
// This is used for activities not explicitly listed in VERSIONED_ACTIVITY_TYPES.
func buildAutoDetectedActivityTypes(definitions: [String: SwaggerSpec.Definition]) -> [String: String] {
    let intentRegex = try! NSRegularExpression(pattern: "^v\\d+([A-Z][A-Za-z0-9]*?)Intent(V(\\d+))?$")

    var latestVersions: [String: Int] = [:]

    for key in definitions.keys {
        let range = NSRange(key.startIndex..<key.endIndex, in: key)
        guard let match = intentRegex.firstMatch(in: key, range: range) else { continue }
        guard let baseRange = Range(match.range(at: 1), in: key) else { continue }
        let baseName = String(key[baseRange])

        var versionNum = 0
        if match.range(at: 3).location != NSNotFound,
            let vNumRange = Range(match.range(at: 3), in: key),
            let num = Int(key[vNumRange])
        {
            versionNum = num
        }

        if (latestVersions[baseName] ?? -1) < versionNum {
            latestVersions[baseName] = versionNum
        }
    }

    var result: [String: String] = [:]
    for (baseName, version) in latestVersions {
        guard version > 0 else { continue }
        let baseActivityType = "ACTIVITY_TYPE_\(snakeCase(baseName).uppercased())"
        result[baseActivityType] = "\(baseActivityType)_V\(version)"
    }
    return result
}
...
func generatePublicClientFile(swagger: SwaggerSpec) -> String {
    ...
    let autoDetectedActivityTypes = buildAutoDetectedActivityTypes(definitions: swagger.definitions)
    ...
    for (endpointPath, pathItem) in sortedPaths {
        ...
        if methodType == "query" {
            ...
            let pinnedConfig = VERSIONED_ACTIVITY_TYPES[unversionedActivityType]
            let activityType =
                pinnedConfig?.activityType
                ?? autoDetectedActivityTypes[unversionedActivityType]
                ?? unversionedActivityType

            let resultKey = operationNameWithoutNamespace + "Result"
            let versionedResultKey: String
            if let pinnedResult = pinnedConfig?.resultKey {
                versionedResultKey = formatDefinitionKey(pinnedResult)
            } else {
                versionedResultKey = latestVersions[resultKey]?.formattedKeyName ?? camelCase(resultKey)
            }
            ...
            let unversionedActivityType = "ACTIVITY_TYPE_\(snakeCase(operationNameWithoutNamespace).uppercased())"
            let activityType =
                VERSIONED_ACTIVITY_TYPES[unversionedActivityType]?.activityType
                ?? autoDetectedActivityTypes[unversionedActivityType]
                ?? unversionedActivityType
