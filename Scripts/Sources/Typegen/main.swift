...
let ONEOF_FIELDS = CodegenConfig.oneOfFields
...
// Determine if a method should have all parameters optional based on Swagger spec
func shouldMakeAllOptional(methodType: MethodType, requestDef: SwaggerSpec.Definition) -> Bool {
    let requiredProps = requestDef.required ?? []
    let sdkInjectedFields: Set<String>
    
    switch methodType {
    case .command, .activityDecision:
        sdkInjectedFields = ["organizationId", "timestampMs"]
    case .query, .noop:
        sdkInjectedFields = ["organizationId"]
    case .proxy:
        sdkInjectedFields = []
    }
    
    // If all required fields are SDK-injected, treat as all-optional
...
func generateSwiftType(name: String, def: SwaggerSpec.Definition) -> String {
    ...
    if def.type == "object", let properties = def.properties {
        let oneOfFieldNames = Set(ONEOF_FIELDS[name] ?? [])
        let hasOneOf = !oneOfFieldNames.isEmpty
        ...
        // Track oneOf field types for the enum generation
        var oneOfCases: [(jsonName: String, swiftType: String, description: String?)] = []

        for (propName, schemaBox) in properties.sorted(by: { $0.key < $1.key }) {
            let schema = schemaBox.value

            // Skip oneOf fields from normal property generation
            if oneOfFieldNames.contains(propName) {
                var swiftType = "Any"
                ...
                oneOfCases.append(
                    (jsonName: propName, swiftType: swiftType, description: schema.description))
                continue
            }
        ...
        // Generate the OneOf enum if this type has oneOf fields
        if hasOneOf {
            output += "    public let oneOf: OneOf\n"
            output += "\n    public enum OneOf: Codable, Sendable {\n"
            for c in oneOfCases {
                if let desc = c.description {
                    output += "        /// \(desc)\n"
                }
                output += "        case \(c.jsonName)(\(c.swiftType))\n"
            }

            // Custom CodingKeys for the enum
            output += "\n        private enum CodingKeys: String, CodingKey {\n"
            for c in oneOfCases {
                output += "            case \(c.jsonName)\n"
            }
            output += "        }\n"
            ...
            for c in oneOfCases {
                output +=
                    "            if let value = try container.decodeIfPresent(\(c.swiftType).self, forKey: .\(c.jsonName)) {\n"
                output += "                self = .\(c.jsonName)(value)\n"
                output += "                return\n"
                output += "            }\n"
            }
            output += "            throw DecodingError.dataCorrupted(\n"
            output +=
                "                DecodingError.Context(codingPath: decoder.codingPath, debugDescription: \"No valid oneOf field found for \(name)\")\n"
            output += "            )\n"
            output += "        }\n"
            ...
            output += "            switch self {\n"
            for c in oneOfCases {
                output += "            case .\(c.jsonName)(let value):\n"
                output += "                try container.encode(value, forKey: .\(c.jsonName))\n"
            }
            output += "            }\n"
            output += "        }\n"
        ...
        if !properties.isEmpty {
            ...
            // Build init params: base props + oneOf
            var initParams: [(label: String, type: String, hasDefault: Bool)] = []
            for key in sortedProps {
                ...
                if !isRequired { swiftType += "?" }
                initParams.append((label: key.swiftName, type: swiftType, hasDefault: !isRequired))
            }
            if hasOneOf {
                initParams.append((label: "oneOf", type: "OneOf", hasDefault: false))
            }

            for (index, param) in initParams.enumerated() {
                let comma = index < initParams.count - 1 ? "," : ""
                if param.hasDefault {
                    output += "        \(param.label): \(param.type) = nil\(comma)\n"
                } else {
                    output += "        \(param.label): \(param.type)\(comma)\n"
                }
            }
            output += "    ) {\n"
            for key in sortedProps {
            ...
            if hasOneOf {
                output += "        self.oneOf = oneOf\n"
            }
            output += "    }\n"
        }

        // For oneOf types, always generate custom CodingKeys and Codable to flatten the enum
        if hasOneOf {
            output += "\n    private enum CodingKeys: String, CodingKey {\n"
            ...
            // Custom init(from:) that decodes base props + delegates oneOf to the enum
            output += "\n    public init(from decoder: Decoder) throws {\n"
            ...
            for key in codingKeys {
                ...
                if isRequired {
                    output +=
                        "        self.\(key.swiftName) = try container.decode(\(swiftType).self, forKey: .\(key.swiftName))\n"
                } else {
                    output +=
                        "        self.\(key.swiftName) = try container.decodeIfPresent(\(swiftType).self, forKey: .\(key.swiftName))\n"
                }
            }
            output += "        self.oneOf = try OneOf(from: decoder)\n"
            output += "    }\n"

            // Custom encode(to:) that encodes base props + flattens oneOf
            output += "\n    public func encode(to encoder: Encoder) throws {\n"
            output += "        var container = encoder.container(keyedBy: CodingKeys.self)\n"
            for key in codingKeys {
                let isRequired = requiredProps.contains(key.jsonName)
                if isRequired {
                    output +=
                        "        try container.encode(self.\(key.swiftName), forKey: .\(key.swiftName))\n"
                } else {
                    output +=
                        "        try container.encodeIfPresent(self.\(key.swiftName), forKey: .\(key.swiftName))\n"
                }
            }
            output += "        try self.oneOf.encode(to: encoder)\n"
            output += "    }\n"
        } else {
...
func generateApiTypes(
    ...
    let namespace =
        swagger.tags?.first(where: { $0.name.contains("Service") })?.name ?? "PublicApiService"
    ...
    for (_, pathItem) in swagger.paths.sorted(by: { $0.key < $1.key }) {
                ...
                for param in parameters {
                        ...
                        let activityType = typeEnum.first
                    {
                        ...
                        if let mapped = VERSIONED_ACTIVITY_TYPES[activityType] {
                            if let versionMatch = mapped.activityType.range(
                                of: "V\\d+$", options: .regularExpression)
                            {
                                versionSuffix = String(mapped.activityType[versionMatch])
                            }
                        }
                        ...
                        let baseActivity =
                            ...
                            .replacingOccurrences(
                                of: "Request(V\\d+)?$", with: "", options: .regularExpression)
                        ...
                        if let versionInfo = latestVersions[resultBase] {
                            if !versionSuffix.isEmpty {
                                ...
                                let candidate = definitions.keys.first {
                                    $0.hasPrefix("v1" + baseActivity + "Result")
        ...
        case .command, .activityDecision:
            ...
            if let paramsRef = requestDef.properties?["parameters"]?.value.ref {
                let isAllOptional = shouldMakeAllOptional(methodType: methodType, requestDef: requestDef)
...
func main() throws {
    print("🚀 Starting Type Generation for Swift...")
    ...
    print(
        "   - Public API: \(publicSpec.paths.count) endpoints, \(publicSpec.definitions.count) definitions"
    )
    print(
        "   - Auth Proxy: \(authProxySpec.paths.count) endpoints, \(authProxySpec.definitions.count) definitions"
    )
    ...
    print("✅ Generated types at: \(OUTPUT_PATH.path)")
