...
enum HpkeHelpers {
    ...
    static func decryptExportBundle(
        ...
        do {
            outer = try JSONDecoder().decode(BundleOuter.self, from: Data(exportBundle.utf8))
        } catch {
            throw CryptoError.decodingFailed(error)
        }
        
        guard try SignatureVerifier.verifyEnclaveSignature(
        ...
        guard let ciphertextHex = inner.ciphertext else {
            throw CryptoError.missingCiphertext
        }
        
        guard let ct = Data(hexString: ciphertextHex),
              let ek = Data(hexString: encappedHex) else {
            throw CryptoError.invalidHexString(ciphertextHex)
        }
    ...
    static func encryptWalletToBundle(
        ...
        let outer: BundleOuter
        do {
            outer = try JSONDecoder().decode(BundleOuter.self, from: Data(importBundle.utf8))
        } catch {
            throw CryptoError.decodingFailed(error)
        }
    ...
    static func hpkeEncrypt(
        ...
        let compressedEncapped = try P256.KeyAgreement.PublicKey(
            x963Representation: sender.encapsulatedKey
