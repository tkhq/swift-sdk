...
extension PasskeyOperationRunner: ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization auth: ASAuthorization) {
        ...
        case let reg as ASAuthorizationPlatformPublicKeyCredentialRegistration:
            do { regCont?.resume(returning: try service.handleRegistrationResult(reg)) }
            catch { regCont?.resume(throwing: error) }
            regCont = nil
