...
extension TurnkeyContext {
  ...
  public func fetchOrCreateP256ApiKeyUser(
    ...
    let apiKeyName =
      params.apiKeyName.isEmpty ? "public-key-user-\(params.publicKey)" : params.apiKeyName
    ...
    let userResp = try await client.getUser(
      TGetUserBody(
        organizationId: orgId,
        userId: newUserId
      ))
  ...
  public func fetchOrCreatePolicies(
    ...
    let createResp = try await client.createPolicies(
      TCreatePoliciesBody(
        organizationId: orgId,
        policies: missing
      ))
