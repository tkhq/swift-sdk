    ...
    private func handleContinueWithEmail() {
        Task {
            do {
                ...
                coordinator.push(AuthRoute.otp(otpId: otpId, contact: email, otpType: OtpType.email, publicKey: publicKey))
            } catch {
    ...
    private func handleContinueWithPhone() {
        Task {
            do {
                ...
                coordinator.push(AuthRoute.otp(otpId: otpId, contact: phone, otpType: OtpType.sms, publicKey: publicKey))
            } catch {
                auth.error = "Failed to send OTP"
            }
        }
