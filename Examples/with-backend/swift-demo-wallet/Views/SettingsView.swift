...
struct SettingsView: View {
    ...
    private func handleEmailUpdate(newEmail: String) {
        Task {
            do {
                let (returnedOtpId, _) = try await auth.sendOtp(contact: newEmail, type: .email)
                otpId = returnedOtpId
                otpContact = newEmail
                otpType = .email
                updateType = .email
    ...
    private func handlePhoneUpdate(newPhone: String) {
        Task {
            do {
                let (returnedOtpId, _) = try await auth.sendOtp(contact: newPhone, type: .sms)
                otpId = returnedOtpId
                otpContact = newPhone
                otpType = .sms
                updateType = .phone
    ...
    private func handleOtpComplete(otpCode: String) async throws {
        ...
        let verificationToken = try await auth.verifyOtp(otpId: otpId, otpCode: otpCode)
